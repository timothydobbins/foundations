...
-   **Continuous** data:
    ...
    -   `descriptives(data=dataframe, vars=c(var1, var2), pc=TRUE, ci=TRUE, splitBy=group_var)` (from `jmv`): Comprehensive descriptive statistics, including percentiles, confidence interval of the mean, and can split by a categorical variable. \[Section 1.14; Section 3.22\]
-   **Categorical** data:
...
-   **Normal** distribution in R \[Section 3.26\]:
    -   Probability of q or less: `pnorm(q, mean, sd)`.
    -   Probability of more than q: `pnorm(q, mean, sd, lower.tail=FALSE)`.

### 7. Confidence Intervals

-   **Mean** (individual data): `descriptives(data=dataframe, vars=variable, ci=TRUE)` (from `jmv`). \[Section 3.27\]
-   **Mean** (summarised data): Use the custom `ci_mean` function provided in the notes, e.g. `ci_mean(n=242, mean=128.4, sd=19.56, width=0.95)`. \[Section 3.28\]
...
-   **One-Sample T-test**: `t.test(dataframe$variable, mu=hypothesised_mean)`. \[Section 4.17\]
